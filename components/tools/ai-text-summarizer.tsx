...
export default function FreeAiAiTextSummarizerTool() {
  ...
  const handleRun = () => {
    ...
    const fewShotConfig = [
    { role: "system", content: "You are a strict, single-purpose utility for summarizing long text into concise bullet points or a single summary sentence. Output ONLY the final processed result. Do NOT include greetings, conversational filler, markdown blocks, or explanations." },
    { role: "user", content: "Next.js is a React framework for building full-stack web applications. You use React Components to build user interfaces, and Next.js for additional features and optimizations." },
    { role: "assistant", content: "Next.js is a React framework for building full-stack web applications with optimizations." },
    { role: "user", content: "Client-side AI runs entirely in the browser memory using quantized models, preventing any data from leaving the local machine. This guarantees 100% privacy and security since no network calls are made to external servers." },
    { role: "assistant", content: "- Client-side AI runs completely inside the browser memory.\n- Guarantees 100% privacy with zero network calls to external servers." }
  ];
    generate([
      ...fewShotConfig,
