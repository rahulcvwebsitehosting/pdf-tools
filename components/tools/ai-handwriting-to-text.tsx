...
export default function FreeAiAiHandwritingToTextTool() {
  ...
  const handleDownloadTxt = () => {
    ...
    link.href = url;
    link.download = "ai-handwriting-to-text.txt";
    link.click();
  };
  ...
  const handleDownloadDocx = () => {
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = "ai-handwriting-to-text.doc";
    link.click();
  };
            ...
            {image && (
              <div className="flex items-center gap-3">
                <button onClick={handleProcess} disabled={!isReady || isOcrRunning} className="btn-primary">
                  {isOcrRunning ? 'Running OCR...' : 'Extract Handwriting'}
                </button>
                {!isReady && (
