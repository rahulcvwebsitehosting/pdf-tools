...
export default function FreeAiAiEmailSubjectAnalyzerTool() {
  ...
  const handleRun = () => {
    ...
    const fewShotConfig = [
    { role: "system", content: "You are a strict, single-purpose utility for evaluating email subject lines for open-rate potential and suggesting improvements. Output ONLY the final processed result. Do NOT include greetings, conversational filler, markdown blocks, or explanations." },
    { role: "user", content: "hi open this" },
    { role: "assistant", content: "Score: 2/10. Issue: Spammy, low interest. Suggestion: Be specific." },
    { role: "user", content: "Save 20% on client-side AI tools today only!" },
    { role: "assistant", content: "Score: 8/10. Strengths: Clear value, urgency.\nSuggestion: \"Exclusive: Save 20% on client-side AI tools today\"" }
  ];
    generate([
      ...fewShotConfig,
