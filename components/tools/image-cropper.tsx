        ...
        {image && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            <div>
              ...
              <input type="number" value={cropX} onChange={(e) => setCropX(parseInt(e.target.value, 10))} className="p-2 border border-border bg-background font-mono text-xs w-full" />
            </div>
            <div>
              <label className="block text-xs font-mono uppercase font-bold text-muted-foreground mb-1">Y offset</label>
              <input type="number" value={cropY} onChange={(e) => setCropY(parseInt(e.target.value, 10))} className="p-2 border border-border bg-background font-mono text-xs w-full" />
            </div>
            <div>
              <label className="block text-xs font-mono uppercase font-bold text-muted-foreground mb-1">Width (px)</label>
              <input type="number" value={cropW} onChange={(e) => setCropW(parseInt(e.target.value, 10))} className="p-2 border border-border bg-background font-mono text-xs w-full" />
            </div>
            <div>
              <label className="block text-xs font-mono uppercase font-bold text-muted-foreground mb-1">Height (px)</label>
              <input type="number" value={cropH} onChange={(e) => setCropH(parseInt(e.target.value, 10))} className="p-2 border border-border bg-background font-mono text-xs w-full" />
            </div>
          </div>
        )}
        {image && (
