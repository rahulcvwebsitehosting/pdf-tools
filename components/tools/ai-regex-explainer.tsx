...
export default function FreeAiAiRegexExplainerTool() {
  ...
  const handleRun = () => {
    ...
    const fewShotConfig = [
    { role: "system", content: "You are a strict, single-purpose utility for explaining regular expressions in simple, understandable terms. Output ONLY the final processed result. Do NOT include greetings, conversational filler, markdown blocks, or explanations." },
    { role: "user", content: "/[a-z]+/i" },
    { role: "assistant", content: "Matches one or more alphabetical characters (case-insensitive)." },
    { role: "user", content: "/^\\d{5}(-\\d{4})?$/" },
    { role: "assistant", content: "- ^ asserts start of line\n- \\d{5} matches exactly 5 digits\n- (-\\d{4})? optionally matches a hyphen followed by 4 digits\n- $ asserts end of line" }
  ];
    generate([
      ...fewShotConfig,
