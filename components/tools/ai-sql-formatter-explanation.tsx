...
export default function FreeAiAiSqlFormatterExplanationTool() {
  ...
  const handleRun = () => {
    ...
    const fewShotConfig = [
    { role: "system", content: "You are a strict, single-purpose utility for formatting SQL queries and explaining the database operations being performed. Output ONLY the final processed result. Do NOT include greetings, conversational filler, markdown blocks, or explanations." },
    { role: "user", content: "select * from users where id=1" },
    { role: "assistant", content: "SELECT * \nFROM users \nWHERE id = 1;\n\nExplanation: Retrieves all columns for the user matching ID 1." },
    { role: "user", content: "select name, email from customers join orders on customers.id=orders.customer_id group by name" },
    { role: "assistant", content: "SELECT name, email \nFROM customers \nINNER JOIN orders ON customers.id = orders.customer_id \nGROUP BY name, email;\n\nExplanation: Joins customer profiles with order histories and aggregates fields by name." }
  ];
    generate([
      ...fewShotConfig,
