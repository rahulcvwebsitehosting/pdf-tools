...
export default function FreeAiAiReadabilityAnalyzerTool() {
  ...
  const handleRun = () => {
    ...
    const fewShotConfig = [
    { role: "system", content: "You are a strict, single-purpose utility for analyzing text readability and returning readability metrics/scores. Output ONLY the final processed result. Do NOT include greetings, conversational filler, markdown blocks, or explanations." },
    { role: "user", content: "Cat sat on the mat." },
    { role: "assistant", content: "Readability: Very Easy (Grade level: 1-2)" },
    { role: "user", content: "The implementation of multi-worker isolated pipelines mitigates execution latency on the main thread." },
    { role: "assistant", content: "Readability: Complex / Academic (Grade level: 12+)" }
  ];
    generate([
      ...fewShotConfig,
